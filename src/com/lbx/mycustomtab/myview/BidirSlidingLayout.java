...
/**
 * 双向滑动菜单框架,布局时，BidirSlidingLayout作为父组件，
 * 其必须包含三个子视图[leftMenu/content/rightMenu]
 * 注意要让左侧菜单和父布局左边缘对齐，右侧菜单和父布局右边缘对齐;
 * 必须要通过方法setScrollEvent(View)给一个View注册touch事件。
 * 可以通过方法setCanShowMenu(int)设置可以显示的菜单
 * @author lbx
...
public class BidirSlidingLayout extends RelativeLayout implements OnTouchListener {
	...
	/**
	 * 显示侧边菜单的一种，表示可以显示左右两侧菜单
	 */
	public static final int CAN_SHOW_LEFT_RIGHT_MENU = 0;
	/**
	 * 显示侧边菜单的一种，表示可以显示左侧菜单
	 */
	public static final int CAN_SHOW_LEFT_MENU = 1;
	/**
	 * 显示侧边菜单的一种，表示可以显示右侧菜单
	 */
	public static final int CAN_SHOW_RIGHT_MENU = 2;
	 /**
	  * 记录当前可以显示的侧边菜单
	  */
	private int canShowMenu;
	/**
	 * 记录当前的滑动状态
	 */
	...
	public BidirSlidingLayout(Context context, AttributeSet attrs) {
		...
		touchSlop = ViewConfiguration.get(context).getScaledTouchSlop();////获得能够进行手势滑动的距离
	}
	/**
	 * 设置可以显示的侧边菜单
	 * @param canShowMenu CAN_SHOW_LEFT_RIGHT_MENU;CAN_SHOW_LEFT_MENU;CAN_SHOW_RIGHT_MENU
	 * 
	 */
	public void setCanShowMenu(int canShowMenu){
	 ...
	 * 
	 * 参考：http://www.cnblogs.com/fengzhblog/p/3175910.html
	 * 执行layout操作时调用onLayout方法。View要给它的每个Child设定size和position。
	 * 拥有Children的子类需要重写onLayout方法并且调用每个Child的layout方法。
	 * 参数changed表示view的size或position发生变化。
	 * 参数l, t, r, b分别表示相对于parent的left, top, right, bottom position。
	 */
	@Override
	 ...
	 * 
	 * @param moveDistanceX--(xMove - xDown);
	 * @param moveDistanceY--(yMove - yDown);
	 *            纵向移动的距离
	 */
	private void checkSlideState(int moveDistanceX, int moveDistanceY) {
		if(canShowMenu == CAN_SHOW_LEFT_RIGHT_MENU){
		...
		}else if(canShowMenu == CAN_SHOW_LEFT_MENU){
			if (isLeftMenuVisible) {
				if (!isSliding && Math.abs(moveDistanceX) >= touchSlop && moveDistanceX < 0) {
					isSliding = true;
					slideState = HIDE_LEFT_MENU;
